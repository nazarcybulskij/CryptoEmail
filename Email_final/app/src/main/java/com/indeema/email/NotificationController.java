...
import com.indeema.email.activity.Welcome;
...
public class NotificationController {
    ...
    NotificationController(Context context, Clock clock) {
        ...
        mGenericSenderIcon = BitmapFactory.decodeResource(mContext.getResources(),
                com.indeema.email.R.drawable.ic_contact_picture);
        mGenericMultipleSenderIcon = BitmapFactory.decodeResource(mContext.getResources(),
                com.indeema.email.R.drawable.ic_notification_multiple_mail_holo_dark);
        mClock = clock;
    ...
    private Notification.Builder createBaseAccountNotificationBuilder(Account account,
        ...
        final Notification.Builder builder = new Notification.Builder(mContext)
                ...
                .setSmallIcon(com.indeema.email.R.drawable.stat_notify_email_generic)
            ...
            if (multipleUnseen) {
                if (messageCursor != null) {
                    final int maxNumDigestItems = mContext.getResources().getInteger(
                            com.indeema.email.R.integer.max_num_notification_digest_items);
    ...
    private static CharSequence getSingleMessageInboxLine(Context context, Message message) {
        ...
        final TextAppearanceSpan notificationPrimarySpan =
                new TextAppearanceSpan(context, com.indeema.email.R.style.NotificationPrimaryText);
        ...
        if (TextUtils.isEmpty(senders)) {
            ...
            final String formatString = context.getResources().getString(
                    com.indeema.email.R.string.multiple_new_message_notification_item);
            final TextAppearanceSpan notificationSecondarySpan =
                    new TextAppearanceSpan(context, com.indeema.email.R.style.NotificationSecondaryText);
    ...
    private static CharSequence getSingleMessageLittleText(Context context, String subject) {
        ...
        final TextAppearanceSpan notificationSubjectSpan = new TextAppearanceSpan(
                context, com.indeema.email.R.style.NotificationPrimaryText);
    ...
    private static CharSequence getSingleMessageBigText(Context context, Message message) {
        ...
        if (TextUtils.isEmpty(subject)) {
            ...
            final String notificationBigTextFormat = context.getResources().getString(
                    com.indeema.email.R.string.single_new_message_notification_big_text);
    ...
    SpannableString getNewMessageTitle(String sender, int unseenCount) {
        ...
        if (unseenCount > 1) {
            title = String.format(
                    mContext.getString(com.indeema.email.R.string.notification_multiple_new_messages_fmt),
                    unseenCount);
        } else {
            title = sender;
        }
    ...
    public void showDownloadForwardFailedNotification(Attachment attachment) {
        ...
        showAccountNotification(account,
                mContext.getString(com.indeema.email.R.string.forward_download_failed_ticker),
                mContext.getString(com.indeema.email.R.string.forward_download_failed_title),
    ...
    public void showLoginFailedNotification(long accountId) {
        ...
        if (account == null) return;
        showAccountNotification(account,
                mContext.getString(com.indeema.email.R.string.login_failed_ticker, account.mDisplayName),
                mContext.getString(com.indeema.email.R.string.login_failed_title),
    ...
    public void showPasswordExpiringNotification(long accountId) {
        ...
        String ticker =
            mContext.getString(com.indeema.email.R.string.password_expire_warning_ticker_fmt, accountName);
        String title = mContext.getString(com.indeema.email.R.string.password_expire_warning_content_title);
    ...
    public void showPasswordExpiredNotification(long accountId) {
        ...
        Intent intent = AccountSecurity.actionDevicePasswordExpirationIntent(mContext,
                accountId, true);
        String accountName = account.getDisplayName();
        String ticker = mContext.getString(com.indeema.email.R.string.password_expired_ticker);
        String title = mContext.getString(com.indeema.email.R.string.password_expired_content_title);
        showAccountNotification(account, ticker, title, accountName, intent,
    ...
    public void showSecurityNeededNotification(Account account) {
        ...
        String accountName = account.getDisplayName();
        String ticker =
            mContext.getString(com.indeema.email.R.string.security_notification_ticker_fmt, accountName);
        String title = mContext.getString(com.indeema.email.R.string.security_notification_content_title);
        showAccountNotification(account, ticker, title, accountName, intent,
